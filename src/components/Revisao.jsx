  ...
  return (
    <div style={{
      ...
      <div style={{ background: 'white', borderRadius: 16, padding: 24, width: '100%', maxWidth: 560 }}>
        <div style={{ fontSize: 15, fontWeight: 600, color: '#222', marginBottom: 16 }}>
          Recortar imagem — {imagem.nome}
        </div>
        <div style={{
          position: 'relative', width: '100%', height: 320,
          background: '#111', borderRadius: 10, overflow: 'hidden', marginBottom: 16
        }}>
          <Cropper
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <input
            type="range" min={0.3} max={3} step={0.05} value={zoom}
            onChange={e => setZoom(Number(e.target.value))}
            style={{ width: '100%', accentColor: '#534AB7' }}
          />
        </div>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'space-between', alignItems: 'center' }}>
          <button onClick={rodar} style={{
            padding: '10px 16px', borderRadius: 8, border: '1px solid #ddd',
            background: '#f5f4fe', color: '#534AB7', fontSize: 14,
...
export default function Revisao({ dados, onEditar, onFinalizar, finalizing }) {
  ...
  function handleCropConfirm(croppedDataUrl) {
    ...
    if (olho === 'OD') {
      const novas = [...imagensOD]
      novas[idx] = { ...novas[idx], preview: croppedDataUrl }
      setImagensOD(novas)
    } else {
      const novas = [...imagensOE]
      novas[idx] = { ...novas[idx], preview: croppedDataUrl }
      setImagensOE(novas)
    }
    setCropTarget(null)
    ...
    <div style={{ minHeight: '100vh', background: '#f5f4fe', padding: '32px 16px' }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        ...
        <Card>
          ...
          <Grid2>
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              <Campo label="Morada" valor={dados.tutor_morada} />
            </div>
          </Grid2>
          <Divider />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              ...
              <div key={campo}>
                ...
                <div style={{
                  marginLeft: 28, padding: '8px 12px', borderRadius: 8,
                  border: '1px solid #eee', background: '#fafafa',
                  fontSize: 13, color: flags[`${campo}_obs`] ? '#555' : '#ccc',
                  minHeight: 36
                }}>
                  {flags[`${campo}_obs`] || '—'}
                </div>
        ...
        <Card>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            {[
              ...
              <div key={olho}>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#888', marginBottom: 10, textAlign: 'center' }}>
                  {label}
                </div>
                {imagens.length > 0 ? imagens.map((img, i) => (
                  <div key={i} style={{ position: 'relative', marginBottom: 10 }}>
                    <img src={img.preview} alt={img.nome}
                      style={{ width: '100%', borderRadius: 10, objectFit: 'cover', border: '1px solid #eee', display: 'block' }} />
                    <button
                      onClick={() => setCropTarget({ olho, idx: i, imagem: { ...img, preview: img.original || img.preview } })}
                      style={{
                        position: 'absolute', bottom: 8, right: 8,
                        background: 'rgba(83,74,183,0.85)', color: 'white',
                        border: 'none', borderRadius: 8, padding: '6px 12px',
                        fontSize: 12, fontWeight: 600, cursor: 'pointer'
                      }}
                    >✂️ Recortar / Rodar</button>
                  </div>
                )) : (
                  <div style={{
                    border: '2px dashed #eee', borderRadius: 10, padding: 24,
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 40 }}>
          ...
          <button onClick={onFinalizar} disabled={finalizing} style={{
            padding: '12px 32px', borderRadius: 10, border: 'none',
            background: finalizing ? '#a9a4e8' : '#1D9E75',
