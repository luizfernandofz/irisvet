    ...
    <div style={{ minHeight: '100vh', background: 'var(--iv-bg)', padding: '32px 16px' }}>
      <div style={{ maxWidth: 700, margin: '0 auto' }}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ background: 'var(--iv-surface)', border: '0.5px solid var(--iv-line)', borderRadius: 12, padding: 32, boxShadow: '0 2px 16px rgba(91,110,88,0.08)', marginBottom: 16 }}>
            ...
            <div style={sectionTitle}>
              Dados profissionais
              <span style={{ fontSize: 10, fontWeight: 400, color: 'var(--iv-ink-muted)', marginLeft: 8, textTransform: 'none', letterSpacing: 0 }}>
          ...
          {sucesso && <div style={{ background: 'var(--iv-sage-light)', color: 'var(--iv-sage-dark)', borderRadius: 8, padding: '10px 12px', fontSize: 13, marginBottom: 16 }}>Perfil actualizado com sucesso.</div>}
        ...
        <div style={{ background: 'var(--iv-surface)', border: '0.5px solid var(--iv-line)', borderRadius: 12, padding: 32, boxShadow: '0 2px 16px rgba(91,110,88,0.08)', marginBottom: 40 }}>
          ...
          {assinaturaUrl ? (
            <img src={assinaturaUrl} alt="Assinatura" style={{ maxWidth: 240, maxHeight: 120, border: '1px solid var(--iv-line)', borderRadius: 8, padding: 8, marginBottom: 12, display: 'block' }} />
          ) : (
            <div style={{ fontSize: 13, color: 'var(--iv-ink-muted)', marginBottom: 12 }}>Ainda não definiste uma assinatura.</div>
          )}
          <label style={{
            display: 'inline-block', padding: '8px 16px', borderRadius: 8, border: '1px solid var(--iv-sage)',
            background: 'var(--iv-sage-light)', color: 'var(--iv-sage)', fontSize: 13, fontWeight: 600, cursor: 'pointer',
          }}>
          ...
          {erroAssinatura && <div style={{ background: 'var(--iv-plum-light)', color: 'var(--iv-plum-dark)', borderRadius: 8, padding: '10px 12px', fontSize: 13, marginTop: 12 }}>{erroAssinatura}</div>}
        </div>
      </div>
    </div>
  )
...
const sectionTitle = { fontFamily: 'var(--iv-font-display)', fontSize: 12, fontWeight: 500, color: 'var(--iv-sage)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 16, marginTop: 8 }
